...
When you already have a CVE identifier and just want the canonical record, use `lookup_cve.py`. It queries the official cve.org (MITRE CVE Services) API directly — no Serper key or agentic loop. An Anthropic key is required so Claude can generate the one-line summary; pair it with the CNA description for a quick read without losing the full detail.
...
- `Summary` — a single concise sentence (≤30 words) generated by Claude that names the affected product, the vulnerability type, and the impact
...
Summary: XZ Utils 5.6.0–5.6.1 contains a malicious backdoor injected via build-time obfuscation, enabling remote unauthorized access on affected Linux distributions running sshd.
